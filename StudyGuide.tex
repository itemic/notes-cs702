...
\includegraphics[width=0.65\textwidth]{graphics/activity_lifecycle}
\end{figure}
...
When an app requests permissions, the system presents a dialog box to the user; the app cannot change the dialog box. When the user responds, the system then invokes \texttt{onRequestPermissionsResult()}. The app must override that method to determine if the permission was granted.\\
...
\includegraphics[width=0.75\textwidth]{graphics/completeunlessopen}
\end{figure}
...
\textbf{Differential privacy.} Scramble data and mix it with other users so that the general pattern can be known, rather than specifics.\\

Identifiers are short-lived, random, and anonymous. Data collection involve buckets, sampling, and differential privacy.\\

There are different types of identifiers:
\begin{itemize}
	\item \textbf{UUID.} Resets on system call. App scope.
	\item \textbf{Vendor ID.} Same for apps from same team ID. Developer scope.
	\item \textbf{Advertising ID.} Unique to all apps in device, can be reset. Device scope.
\end{itemize}

In addition, MAC address randomization is used.

\subsubsection{Differential privacy}
A formal way of protecting individual privacy in a group by adding noise. The results from the database with or without an individual must not disclose privacy information of the individual.\\

Add statistically biased noise to data bit which averages out over a large number of data points.

\begin{shaded}
A randomized function $F$ is $\epsilon$-differentially private if for all data sets $D_1$ and $D_2$ differing at most by one element, and all $S \subseteq \text{Range}(F)$.

$$\Pr[(F(D_1) \in S)] = e^{\epsilon} \times \Pr[(F(D_2)\in S)]$$	

where $e = 1 + \epsilon$.
\end{shaded}

The noise to add should be a function of $\delta F$ (maximum difference in result) and $\epsilon$. The probabilities of results \textit{with or without} an individual are almost the same. \\

Use a probability distribution to generate noise. The type of distribution depends on $F$:
\begin{itemize}
	\item $F$ is \textbf{numeric.} Laplace distribution
	\item $F$ is \textbf{categorical.} Exponential distribution
\end{itemize}


In iOS, noise is added to individual data inside the device, and the noisy data is sent to Apple. Again, the trick is to add statistically biased noise to data bit which averages out over a large number of data points.

\begin{figure}[h]
\centering
\caption{Key Hierarchy}
\includegraphics[width=0.75\textwidth]{graphics/diffpriv}
\end{figure}

\subsubsection*{How it works}
\begin{itemize}
	\item Hash the value so that it becomes a fixed length.
	\item Add noise by flipping some bits.
	\item By average all the values you will end up with the original hash.
	\item The probability of changing 0 to 1 and 1 to 0 is less than the probability of not changing them.
\end{itemize}


\subsection{Enterprise security in iOS}
Enterprises have to manage devices that may be used to access or store sensitive enterprise data that could be put at risk. Devices could be misplaced, lost, or stolen.\\

There is a need to control devices in a number of ways, such as requiring auto-lock, managing data/app restrictions (e.g. no camera access), and applying rules (passcode rules, remote wipe). In the case of BYOD, users can do unsafe things such as installing unauthorized third-party apps.

\subsubsection*{Na\"{i}ve approach}
Manual configurations by IT admins. This is labor-intensive and error-prone.

\subsubsection*{iOS configuration management}
iOS devices are managed through the creation and installation of configuration profiles, that contain settings configured by an administrator for installation on a user's device. This is managed using iPhone Configuration Utility and is a \textbf{mobile device management} (MDM) system.\\

A configuration profile is used to set up organization's security policy. It is an XML plist that can be optionally signed and encrypted.\\
...
\begin{itemize}
	...
	\item creating organization
	\item other fields
\end{itemize}
\end{multicols}

The payloads are the most important part:
\begin{itemize}
	\item \textbf{Removal password.} Password to remove locked profiles from the device. This can be set to \textit{Never Remove} so the device has to be cleared to remove it.
	\item \textbf{Passcode policy.} Defines whether passcode is required, and how complex it must be
	\item \textbf{E-mail.} Configures user's email account
	\item \textbf{Restrictions.} Restricts user from certain actions and apps
	\item \textbf{Calendar subscription.} Subscribes user to a shared calendar
	\item \textbf{VPN.} Specifies a VPN connection
	\item \textbf{Wi-Fi.} Configures device to use specified network
\end{itemize}

\subsubsection*{iPhone Configuration Utility}
A GUI utility to allow admins to create and manage config profiles. A self-signed certificate is created on the installing device on first run. It is installed onto iOS devices via USB connections. The device specific certificates signed by the root certificate are transferred to the iOS device for facilitating digital signature.\\

This can be updated by USB, email, or web-server. However, it is not scalable and only manages a limited number of devices.\\

\textbf{Apple Configurator} is a more user-friendly version.

\begin{table}[h]
\centering
\caption{Configuration utilities comparison}
\begin{tabular}{|p{6cm}|l|l|}
\hline
 & \textbf{Apple Configuration} & \textbf{iPhone Configuration Utility} \\ \hline
 Can run on Windows & \xmark & \checkmark \\ \hline
 Can apply profiles and apps to multiple devices at once & \checkmark & \xmark \\ \hline
 Can automatically update iOS and profiles & \checkmark & \xmark \\ \hline
 Can backup devices & \checkmark & \xmark \\ \hline
  
\end{tabular}
\end{table}

\subsubsection{MDM systems}
MDM systems are used to manage a large number of devices. Apple offers an MDM system in Lion Server through the Profile Manager service. This works well for workgroups and small/medium-sized enterprises.\\

Larger organizations may find using a commercial third-party MDM solution to be better.\\

Distributing via MDM requires three components: an iOS device, the organization's MDM server, and Apple Push Notification Service.\\

Devices inform the APNS which topics they are subscribing to. The MDM server tells the APNS to publish a notification which is sent to subscribed devices. The device then establishes a connection to the MDM server over HTTPS. A remote wipe command can be initiated by MDM, Exchange, or iCloud.\\

Enterprise provisioning profiles can be loaded with the configuration profile as well, allowing in-house enterprise apps to be distributed (through MDM or OTA). These profiles have to be renewed annually.\\

Kill switches are a concern for some companies. Some companies don't trust software restrictions so purchase special hardware.


\subsection{iOS jailbreaking}
\textbf{Jailbreaking} is removing certain security restrictions on iOS devices by manipulating the software stack. This allows full access to the OS and the filesystem. Installers such as Cydia allow the installation of apps not signed by Apple.\\

Jailbreaking is legal in the US (DMCA 2010), but can void warranty.\\

People jailbreak to install third-party software, to bypass carrier locks, and to evaluate security and discover vulnerabilities.\\

The goal is to be able to run unsigned code and change the filesystem.\\
...
There are two main types of jailbreak persistences:
\begin{itemize}
	\item \textbf{Tethered jailbreaks.} The jailbreak disappears upon device restart. (e.g. RedSn0w) The device has to be connected to a computer. The kernel is patched each time after reboot. Exploit against privileged code (USB device driver), exploit against unprivileged code and privilege escalation exploit. 
	\item \textbf{Untethered jailbreaks.} Permanent. (e.g. Pandora by Alibaba for iPhone X) Harder to do as it needs to make changes to boot sequence. Used to be easy when there were bootrom vulnerabilities. Now requires multiple exploits: a tethered jailbreak to allow unsigned code to be executed, and then find ways to add exploits to the root filesystem; privileges need to be escalated to patch kernel
\end{itemize}

There are also semi-tethered and semi-untethered variants.
...
\begin{itemize}
	...
	\item ROP
	\item Disable DEP
	\item Load custom kernel
	\item Modify running kernel
\end{itemize}

Kernel patch:
\begin{itemize}
	\item Modify file system
	\item Disable code signing
	\item Change sandbox behavior
	\item Disable MAC policies
\end{itemize}
\end{multicols}

The location of the vulnerability impacts the access level, which vary from hardware to software. There are three levels of exploits: Bootrom, iBoot, and userland.

\subsubsection*{Bootrom level}
Vulnerabilities inside the hardware. From the point of view of a jailbreaker, these are the most powerful vulnerabilities as they enable jailbreakers to modify the whole bootchain.\\

These cannot be fixed by any software update, and only within the next hardware revision. Famous exploits are \textbf{SHAtter} and \textbf{Limera1n}. These affected all A4-based devices.
...
\begin{shaded}
	...
	\begin{itemize}
		\item Connect device via USB and force into DFU mode.
		\item Use overflow to run code that can patch signature verification code. 
		\item Boot of ramdisk, then run patched version of the low-level bootloader and kernel that allow execution of unsigned code)
	\end{itemize}
	This is a \textit{tethered} jailbreak but can be used to create an untethered one.
\end{shaded}

\subsubsection*{iBoot level}
Vulnerabilities inside iBoot. This is as powerful as that of the bootrom in terms of features as it is still early in the boot process.\\

iBoot is not baked into hardware so can be fixed by a software upgrade. iH8sn0w is an A5 iBoot exploit.

\subsubsection*{Userland level}
Vulnerabilities in userland processes. This is less powerful and is easier to fix. These processes can run with any permissions (root user - system processes, mobile users and user apps).\\

Two exploits needed. First to achieve arbitrary code execution, and second to escalate privileges.

\begin{shaded}
	\textbf{Jailbreakme 2.0: Star}\\
	
	Userland exploit where you can go to a website to jailbreak without tethering or rebooting. This was a stack overflow attack when MobileSafari handled a particular font (FreeType parser when rendering PDFs).	 This allowed exploit to mount ROP within MobileSafari. \\
	
	The payload also exploited another vulnerability to increase level of access. This was an integer overflow in an \texttt{IOSurface} property, a private framework to share graphics between processes. This attack allowed code execution in the kernel.\\
	
	Code signing was disabled from the kernel. The ROP downloaded an unsigned library that jailbroke the device.
\end{shaded}

\subsubsection*{Dangers of jailbreaking}
Jailbreaking disables security features:
\begin{multicols}{2}
\begin{itemize}
	\item Allows unsigned code to run
	\item Increases attack surface
	\item Adds shell and other utilities
...
\begin{shaded}
	...
	Many jailbroken phones had an SSH server installed, and the root password (\texttt{alpine}) was not changed. The SSH server wasn't sandboxed.\\
	
	Originally the worm just changed the wallpaper, but was then used to lock the phone, steal data, and create botnets.
\end{shaded}
